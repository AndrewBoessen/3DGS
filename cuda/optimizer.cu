...
__global__ void adam_kernel(const int N, float *__restrict__ param, const float *__restrict__ param_grad,
                            ...
                            const float b2, const float eps, const float b1_t_corr, const float b2_t_corr) {
  ...
  float m_hat = register_exp_avg / b1_t_corr;
  float v_hat = register_exp_avg_sq / b2_t_corr;
...
void adam_step(float *params, float *const param_grads, float *exp_avg, float *exp_avg_sq, const float lr,
  ...
  adam_kernel<<<blocks, threads, 0, stream>>>(N, params, param_grads, exp_avg, exp_avg_sq, lr, b1, b2, eps, b1_t_corr,
                                              b2_t_corr);
}
