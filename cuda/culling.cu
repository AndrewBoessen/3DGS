...
__global__ void scatter_add_gradients_kernel(const float u_scale, const float v_scale, const float *d_grad_uv_culled,
  ...
  if (d_mask[idx]) {
    ...
    const float u = d_grad_uv_culled[culled_idx * 2 + 0] * u_scale;
    const float v = d_grad_uv_culled[culled_idx * 2 + 1] * v_scale;
    const float norm = sqrt(u * u + v * v);

    atomicAdd(&d_uv_grad_accum_full[idx], norm);
