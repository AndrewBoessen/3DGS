...
void rasterize_image(const int num_gaussians, const Camera &camera, const ConfigParameters &config,
  ...
  filter_gaussians_by_mask(num_gaussians, cuda.d_mask, cuda.d_xyz, cuda.d_rgb, cuda.d_opacity, cuda.d_scale,
                           cuda.d_quaternion, cuda.d_uv, cuda.d_xyz_c, cuda.d_xyz_culled, cuda.d_rgb_culled,
                           cuda.d_opacity_culled, cuda.d_scale_culled, cuda.d_quaternion_culled, cuda.d_uv_culled,
                           cuda.d_xyz_c_culled, &pass_data.num_culled);

  if (pass_data.num_culled == 0) {
    return; // No Gaussians in view
  }

  // Step 3: Compute Covariance and Conics
  ...
  render_image(cuda.d_uv_culled, cuda.d_opacity_culled, pass_data.d_conic, cuda.d_rgb_culled, 1.0f,
