...
TEST_F(CudaBackwardKernelTest, ConicBackward) {
  ...
  // Host data
  std::vector<float> h_J = {1.0, 0.0, -0.5, 0.0, 1.0, -0.2,                               // Gaussian 1
                            0.8, 0.1, -0.3, 0.2, 0.9, -0.1};                              // Gaussian 2
  std::vector<float> h_sigma = {1.0, 0.1,  0.2, 2.0, 0.3,  3.0,                           // Gaussian 1
                                0.5, -0.1, 0.1, 1.5, -0.2, 2.5};                          // Gaussian 2
  std::vector<float> h_T = {0.8, -0.6, 0.0, 0.1, 0.6, 0.8, 0.0, 0.2, 0.0, 0.0, 1.0, 0.3}; // Camera extrinsics
  std::vector<float> h_conic_grad_out = {0.1, 0.2, 0.3, -0.1, -0.2, -0.3};
  std::vector<float> h_J_grad_in(N * 6);
  std::vector<float> h_sigma_grad_in(N * 6);

  // Device data
  auto d_J = device_alloc<float>(N * 6);
  auto d_sigma = device_alloc<float>(N * 6);
  auto d_T = device_alloc<float>(12);
  ...
  // Run kernel
  compute_conic_backward(d_J, d_sigma, d_T, d_conic_grad_out, N, d_J_grad_in, d_sigma_grad_in, 0);
  CUDA_CHECK(cudaDeviceSynchronize());
  ...
  auto forward_conic = [&](const std::vector<float> &J_in, const std::vector<float> &sigma_in,
    ...
    std::vector<float> conic(N * 3);
    float W[9] = {T_in[0], T_in[1], T_in[2], T_in[4], T_in[5], T_in[6], T_in[8], T_in[9], T_in[10]};
    ...
    for (int i = 0; i < N; ++i) {
      ...
      for (int r = 0; r < 3; ++r) {
        for (int c = 0; c < 3; ++c) {
          WS[r * 3 + c] = 0;
          for (int k = 0; k < 3; ++k)
            WS[r * 3 + c] += W[r * 3 + k] * S[k * 3 + c];
        }
      }

      float Sc[9]; // Sigma_cam = W * S * W^T
      for (int r = 0; r < 3; ++r) {
        for (int c = 0; c < 3; ++c) {
          Sc[r * 3 + c] = 0;
          for (int k = 0; k < 3; ++k)
            Sc[r * 3 + c] += WS[r * 3 + k] * W[c * 3 + k]; // W^T
        }
      }

      const float *J_ptr = J_in.data() + i * 6;
      float M[6]; // M = J * Sc
      M[0] = J_ptr[0] * Sc[0] + J_ptr[1] * Sc[3] + J_ptr[2] * Sc[6];
      M[1] = J_ptr[0] * Sc[1] + J_ptr[1] * Sc[4] + J_ptr[2] * Sc[7];
      M[2] = J_ptr[0] * Sc[2] + J_ptr[1] * Sc[5] + J_ptr[2] * Sc[8];
      M[3] = J_ptr[3] * Sc[0] + J_ptr[4] * Sc[3] + J_ptr[5] * Sc[6];
      M[4] = J_ptr[3] * Sc[1] + J_ptr[4] * Sc[4] + J_ptr[5] * Sc[7];
      M[5] = J_ptr[3] * Sc[2] + J_ptr[4] * Sc[5] + J_ptr[5] * Sc[8];

      // Conic = M * J^T
      conic[i * 3 + 0] = M[0] * J_ptr[0] + M[1] * J_ptr[1] + M[2] * J_ptr[2];
      conic[i * 3 + 1] = M[0] * J_ptr[3] + M[1] * J_ptr[4] + M[2] * J_ptr[5];
      conic[i * 3 + 2] = M[3] * J_ptr[3] + M[4] * J_ptr[4] + M[5] * J_ptr[5];
    }
    return conic;
  };
  ...
  auto compute_loss = [&](const std::vector<float> &conic) {
    float loss = 0.0f;
    for (size_t i = 0; i < conic.size(); ++i) {
      loss += conic[i] * h_conic_grad_out[i];
    }
    return loss;
  };
  ...
  for (int i = 0; i < N * 6; ++i) {
    ...
    float loss_p = compute_loss(forward_conic(J_p, h_sigma, h_T));
    float loss_m = compute_loss(forward_conic(J_m, h_sigma, h_T));

    float numerical_grad = (loss_p - loss_m) / (2 * h);
    EXPECT_NEAR(h_J_grad_in[i], numerical_grad, 1e-3);
  }
  ...
  for (int i = 0; i < N * 6; ++i) {
    ...
    float loss_p = compute_loss(forward_conic(h_J, sigma_p, h_T));
    float loss_m = compute_loss(forward_conic(h_J, sigma_m, h_T));

    float numerical_grad = (loss_p - loss_m) / (2 * h);
    EXPECT_NEAR(h_sigma_grad_in[i], numerical_grad, 1e-3);
  }
